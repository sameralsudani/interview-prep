...
function sumAll(arr) {
    ...
    let sum =0;
    for (let i = min; i < max; i++) {  // we are looping through from min going up to max (max included)
       sum +=1;    
    }
    return sum
}
...
function diffArray(arr1, arr2) {
    const union =[]
    for (let i = 0; i < arr1.length; i++) {  // we are looping through arr1 from arr1[0] going up to arr1[3] (arr1[3])
        const currentEl = arr1[i];
        if (!union.includes(currentEl)) { // if currentEl is not included in union array  
            union.push(currentEl)  // then add it to union
        } // else skip and go back to line 31
        
    }
    for (let i = 0; i < arr2.length; i++) {  // we are looping through arr2 from arr2[0] going up to arr2[3] (arr2[3])
    ...
    for (let i = 0; i < union.length; i++) { //we are looping through union from union[0] going up to union[3] (union[3])
        ...
        } else if (arr2.includes(currentEl) && !arr1.includes(currentEl)) { // if currentEl is in arr2 and not in arr1
            symDiff.push(currentEl) // then push currentEl to symDiff
        }
